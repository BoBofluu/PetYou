...
<body>
	...
	<div class="outContainer">
		<div class="search">
			...
			<input type="text" value="${object}" id="object" hidden>
			...
			<input hidden id="locationValue" value="${location}" />
			<div class="checkboxObject">
			...
			<div>
				...
				<button id="clearAll" type="button" class="btn btn-danger btn-sm">清除</button>
			</div>
		</div>
			<input hidden class="shopType" value="${shopType}" />
	...
	<!-- outContainer -->
	<c:forEach items="${time}" var="tim" varStatus="sta">
		<input type="text" value="${tim}" id="week${sta.index}" hidden>
	</c:forEach>
	<script>
	//保留使用者前一個頁面針對「對象」的選則
	...
	//透過c:foreach取得使用者選擇的time
	for(let i = 0; i < ${size}; i++){
		//得到選擇到的值之後，再到原本的checkbox將上個頁面選擇的選項checked
		var weekValue = $("#week" + i).val();
		$("#" + weekValue).prop("checked", true);
	}
	console.log($(".shopType").val())
	
	//保留使用者前一個頁面針對「地區」的選則
	$("#location").val($("#locationValue").val()).attr("selected","true");
