...
<!-- 					<li id="onepunch" style="cursor: pointer;"><i class="fa fa-hand-peace-o" aria-hidden="true"></i>一鍵輸入</li> -->
				</ul>
			</div>
				...
				<div id="divaccount">
					...
					<div class="form-table">
						...
						<div class="form-item fih">
							...
							<div class="form-controler">
								<form:input  type="text" class="input-holder" id="password" name="password"
									placeholder="輸入密碼" autocomplete="off" path="password"
				...
				<div id="divpersonal">
					...
					<div class="form-table">
						...
						<div class="form-item fih">
							...
							<div class="form-controler">
								<form:input type="text" path="birthday" class="input-holder" id="birthday" name="birthday"
...
<!-- 							<div class="form-controler"> -->
<%-- 								<form:input type="text"  path="address" class="input-holder"  --%>
<%-- 									id="address" name="address" placeholder="輸入地址" --%>
<%-- 									autocomplete="off"   --%>
<%-- 									/> --%>
<!-- 							</div> -->
<!-- 						</div> -->
					...
					<div class="form-table">
						<div class="form-item" style="width:400px;margin:15px auto;">
							<div style="position:relative;" >
								<form:input type="file" style="position:absolute;opacity:0;width:365px;height:191px;cursor: pointer;" id="memberImage" path="memberImage" accept=".jpg,.png,.jpeg"/> 
								<img src="${member.profilePicture}" id="ppimg"  style="width:365px;"> 
							</div>
						</div>
<!-- 						<div class="form-item"> -->
<!-- 							<div class="form-controler"> -->
...
<%--          							<img src="${member.profilePicture}" id="ppimg"  class="salonIcon">		 --%>
<!--        							</div>	 -->
<!-- 						</div> -->
					</div>
						...
						<div class="form-item">
							<div class="form-controler">
									<form:textarea name="aboutMe" rows="10" cols="50" 
				  						id="usertextarea" onkeyup="wordsTotal()" class="input-holder"
				  						style="resize:none;width:600px;height:200px;text-align:left;padding-left:5px;margin-left: 50px"
				  						placeholder=" 請輸入內容..."
				  						path="aboutMe"/><br>
				  					<span style="margin-left: 50px"></span>字數統計：<span id="display">0</span>
							</div>
						</div>
					<div class="form-item fih">
							<div style="display:flex">
								...
								<input type="submit" class="btnAdd"  value="送出"
									style="margin-left:638px;" id="pageAccount"
	...
	var genderValue = $("#genderValue").val("${member.gender}");
	if(genderValue = "男"){
		$("#r1").attr("checked",true);
	}else if(genderValue = "女"){
		$("#r2").attr("checked",true);
	}else if(genderValue = "無"){
		$("#r3").attr("checked",true);
	}
	$("#birthday").val("${member.birthday}");
	$("#identityCard").val("${member.identityCard}");
	
	var oldAddress = '${member.address}';
	var oldCity = oldAddress.substring(0,3);

	var township = "";
	if(oldCity == "臺北市"){
		$("#city option[value='臺北市']").attr('selected', 'selected');
		initSelect($("#township")[0],taipei);
		taipei.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		});
		$("#township option[value='"+township+"']").attr('selected', 'selected');
	}else if(oldCity == "新北市"){
		$("#city option[value='新北市']").attr('selected', 'selected');
		initSelect($("#township")[0],newTaipei);
		newTaipei.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='桃園市']").attr('selected', 'selected');
		initSelect($("#township")[0],taoYuan);
		taoYuan.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='臺中市']").attr('selected', 'selected');
		initSelect($("#township")[0],taichung);
		taichung.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='臺南市']").attr('selected', 'selected');
		initSelect($("#township")[0],tainan);
		tainan.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='高雄市']").attr('selected', 'selected');
		initSelect($("#township")[0],kao);
		kao.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='新竹縣']").attr('selected', 'selected');
		initSelect($("#township")[0],hsinChu);
		hsinChu.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='苗栗縣']").attr('selected', 'selected');
		initSelect($("#township")[0],miao);
		miao.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='彰化縣']").attr('selected', 'selected');
		initSelect($("#township")[0],chungHua);
		chungHua.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='南投縣']").attr('selected', 'selected');
		initSelect($("#township")[0],nanTou);
		nanTou.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='雲林縣']").attr('selected', 'selected');
		initSelect($("#township")[0],yunLin);
		yunLin.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='嘉義縣']").attr('selected', 'selected');
		initSelect($("#township")[0],chiaYi);
		chiaYi.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='屏東縣']").attr('selected', 'selected');
		initSelect($("#township")[0],pinTong);
		pinTong.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='宜蘭縣']").attr('selected', 'selected');
		initSelect($("#township")[0],yiLan);
		yiLan.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='花蓮縣']").attr('selected', 'selected');
		initSelect($("#township")[0],huaLian);
		huaLian.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='臺東縣']").attr('selected', 'selected');
		initSelect($("#township")[0],taiTong);
		taiTong.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='澎湖縣']").attr('selected', 'selected');
		initSelect($("#township")[0],ponHu);
		ponHu.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='金門縣']").attr('selected', 'selected');
		initSelect($("#township")[0],kingMen);
		kingMen.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='連江縣']").attr('selected', 'selected');
		initSelect($("#township")[0],lianChian);
		lianChian.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='基隆市']").attr('selected', 'selected');
		initSelect($("#township")[0],keeLung);
		keeLung.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='新竹市']").attr('selected', 'selected');
		initSelect($("#township")[0],hisnChuCity);
		hisnChuCity.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
		...
		$("#city option[value='嘉義市']").attr('selected', 'selected');
		initSelect($("#township")[0],chiaYiCity);
		chiaYiCity.forEach(function(value){
			if(oldAddress.substring(3,6) == value)
				township = value;
			else if(oldAddress.substring(3,5) == value)
				township = value;
	...
	var	lastaddress	= (oldAddress.replace(oldCity,"")).replace(township,"");

	$("#lastaddress").val(lastaddress);
	$("#address").val("${member.address}");
	$("#phone").val("${member.phone}");
	$("#memberImage").val("${member.memberImage}");
	$("#usertextarea").val("${member.aboutMe}");
	document.getElementById('display').innerHTML = $("#usertextarea").val().length;
	var prevEmail = $("#email").val();
	var prevAccount = $("#account").val();
	var prevIdc = $("#identityCard").val();
...
// =================================================  check_element() ===========================================================	
		function check_element(){
			...
			const mySet = new Set();
			var message = "";
			if(prevEmail == document.getElementById("email").value){
				emailField = true;
			}
			if(!emailField){
				mySet.add(1);
			...
			if(prevAccount == document.getElementById("account").value){
				accountField = true;
			}
			if(!accountField){
				mySet.add(1);
