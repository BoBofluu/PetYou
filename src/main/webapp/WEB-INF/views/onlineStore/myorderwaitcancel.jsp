		...
		<c:choose>
			<c:when test="${userAccount eq 'Admin' }">
				...
				<h1 style="margin-left: 21%;">
					會員退貨申請 <span style="color: black"> <i
	...
	<div id="app">
		<div class="container">
			<div class="item_header shadow">
				...
				<div class="price">申請人</div>
				<div class="count">地址</div>
				<div class="amount">電話</div>
				<div class="operate">訂單金額</div>
				<c:choose>
					<c:when test="${userAccount eq 'Admin' }">
						<div class="memberid">會員編號</div>
						<div class="memberid">申請單號</div>
					</c:when>
					<c:otherwise>
						<div class="controll">訂單狀態</div>
					</c:otherwise>
				</c:choose>
			...
			<c:forEach var='m' items='${myorder}'>
				<div class="item_header  item_body shadow">
					...
					<c:choose>
						<c:when test="${userAccount eq 'Admin' }">
							...
							<div class="memberid">${m.oid}</div>
							
						</c:when>
						<c:otherwise>
							<div class="operate">
								<button id="${m.orderID}" class="red button">${m.state}</button>
							</div>
						</c:otherwise>
					</c:choose>
				...
				<div class="item_header  item_body shadow">
					...
					<c:choose>
						...
						<c:otherwise>
							<div class="operate">
								<button id="${m.oid}" class="red button">${m.state}</button>
							</div>
						</c:otherwise>
					</c:choose>
