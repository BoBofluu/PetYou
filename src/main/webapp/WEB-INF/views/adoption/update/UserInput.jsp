...
<body>
	...
	<div class="contactInfo">
		...
		<div class="insert">
			...
			<span class="spanStyle">聯絡信箱：</span> <input type="text"
				readonly class="column email emailSpan" id="emailO" />
		</div>
		...
		<div class="insert">
			...
			<span class="spanStyle">會員姓名：</span> <input onblur="family()"
				type="text" value="" name="lastName" required="required"
				class="inputBySelf lastNameSame" style="width: 100px" /> <input
				onblur="family()" type="text" value="" name="firstName"
				required="required" style="width: 120px"
				class="inputBySelf firstNameSame" /> <span id="nameCheck"></span>
		</div>

		<div class="insert">
			<button type="button" class="btn btn-warning"
				style="font-weight: 800; color: snow; width: 55px; height: 30px; font-size: 18px; padding: 0px;">必填</button>
			<span class="spanStyle">連絡電話：</span> <input type="text" value=""
				name="cellPhone" class="inputBySelf cellPhoneSame"
				onblur="phoneCheck()" /><span id="phoneW"></span>
		</div>

		<div class="insert">
			<span class="spanStyle" style="margin-left: 70px">收養地址：</span> <input
				type="text" onblur="addressCheck()" value="" name="address"
				class="inputBySelf addressSame" /> <span id="addressC"></span>
		</div>
	...
	<script>
		...
		xhr
				.open("GET", "<c:url value='/getMemberJson/" + member + "' />",
						true);
		xhr.send();
		...
		// 		$('.contactInfo').width(screen.width * 0.8);
		// 		$('.contactInfo').height(screen.height * 0.9);
		...
		function insert() {
			...
			if (noError >= 5 || noError == 0) {
				if ($(".emailSame").val().length <= 0
					...
					window.location.href = "<c:url  value='/addContact/"
							+ $(".emailSame").val() + "/"
							+ $(".lastNameSame").val() + "/"
							+ $(".firstNameSame").val() + "/"
							+ $(".cellPhoneSame").val() + "/"
